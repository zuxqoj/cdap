  ...
  .controller('DAGPlusPlusCtrl', function MyDAGController(jsPlumb, $scope, $timeout, DAGPlusPlusFactory, GLOBALS, DAGPlusPlusNodesActionsFactory, $window, DAGPlusPlusNodesStore, $rootScope, $popover, uuid, DAGPlusPlusNodesDispatcher, NonStorePipelineErrorFactory, AvailablePluginsStore, myHelpers, HydratorPlusPlusCanvasFactory, HydratorPlusPlusConfigStore, HydratorPlusPlusPreviewActions, HydratorPlusPlusPreviewStore) {
    ...
    vm.selectionBox = {
      ...
      move: ({added, removed}) => {
         ...
         * 
         * I/P : nodes added , nodes removed We already have selected nodes.
         * 1. We iterate through selected nodes and remove the nodes that are removed from selection
         * 2. Create a selected nodes map with nodeid as key
         * 3. Get the adjacency map for the current graph
         * 4. Then iterate through selected nodes and for each node get all nodes connected to it from the adjacency map
         * 5. In the iteration if both the current selected node and the nodes connected to it are
